...
public class UsdtcoindClientFactory {
	 ...
	 * @throws IOException
	 */
	public UsdtcoindClientFactory(URL url, String username, String password)
			throws IOException {
		String cred = Base64
	...
	public UsdtcoindClientFactory(URL url, String username, String password,int connectionTimeoutMillis,int readTimeoutMillis)
			throws IOException {
		String cred = Base64
				.encodeBytes((username + ":" + password).getBytes());
		Map<String, String> headers = new HashMap<>(1);
	...
	public UsdtcoindClientFactory(String path, final List<String> cmd) throws IOException{
		if (isWindows()){
		...
		for (int i = 10; i > 0; i--){
			try{
				ProxyUtil.createClientProxy(UsdtcoindInterface.class.getClassLoader(),
						UsdtcoindInterface.class, client).omni_getinfo();
				success = true;
			}catch(Exception e){
				try {
		...
		if (!success){
			throw new IOException("could not connect to usdtcoind");
		}
	}
	
	public UsdtcoindInterface getClient() {
		return ProxyUtil.createClientProxy(
				UsdtcoindInterface.class.getClassLoader(),
				UsdtcoindInterface.class, client);
	}
